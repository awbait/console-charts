...
### Changed
- Аутентификация: убраны упоминания dev-режима. `config.go` теперь по умолчанию
  `AUTH_MODE=oidc`, dev-режим отклоняется при старте, поэтому в NOTES убрана
  плашка-предупреждение про не-oidc, а README уточнён.
