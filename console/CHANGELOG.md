...
- **Обновили сабчарт `ingress-gateway` до `9.2.0`.** Точка входа портала
  теперь использует общие правила размещения подов из `gatewayScheduling`.
  Для установки портала значения `console` менять не требуется, точка входа
  создаётся как прежде.

## [0.9.4] - 2026-09-25

### Added

- Добавили SVG-иконку на основе знака Console без фона и с общей толщиной линий.

### Changed

- **Сабчарт `ingress-gateway` обновлён до `9.1.0`.** Новые настройки размещения
  пода точки входа по нодам по умолчанию пустые. Значения `console` менять
  не нужно, точка входа портала создаётся как раньше.

## [0.9.3] - 2026-09-10

### Changed
- Пин сабчарта `ingress-gateway` переехал на `9.0.0` следом за самим сабчартом.
  Ломающая часть той версии сюда не достаёт: тег проекта чарт `console` задаёт
  сам в `ingressGateway.identity`, а не получает от портала.

## [0.9.2] - 2026-09-09

### Changed
- Пин сабчарта `ingress-gateway` переехал на `8.0.0` следом за самим сабчартом.
  На значения портала это не влияет: точка входа `http` укладывается в новое
  ограничение имени, а сертификатов чарт `console` не заказывает.
