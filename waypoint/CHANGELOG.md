...
- **Варианты списков в форме заказа подписаны словами.** «Кластер» предлагает
  «techsec-dev (tco)», а не «tco», «Область перехвата» - «Трафик к сервисам
  (service)». Расшифровка кодов ушла из описаний под полями в сами варианты.

## [2.0.2] - 2026-08-24

### Changed
- **Поля в форме заказа подписаны по-человечески.** Раньше подписью служило имя
  ключа: `name`, `for`, `instance`. Теперь это «Имя вейпоинта», «Область
  перехвата», «Инстанс», а под полем сказано, зачем оно нужно.
...
### Removed
- **Поле `enabled` убрано из формы заказа.** В `values.yaml` оно работает
  как раньше: при `false` `Gateway` не создаётся.
