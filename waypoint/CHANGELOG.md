...
- Добавлена SVG-иконка прохождения трафика через узел в едином стиле с ingress и egress.
- **Документ вида `view.schema.json` для портала.** В заказе спрашивается один
  вейпоинт: имя, область перехвата и namespace, которым разрешено привязываться;
  теги окружения и проекта портал проставляет сам. Автомасштабирование и ресурсы
  правятся после заказа, из меню «Действия».

### Changed

- Увеличили символ, убрали подложку и разделили стрелку, узел и контур зазорами, сохранив общую толщину линий и читаемость в обеих темах.

## [2.3.0] - 2026-09-11

### Added
- **Вейпоинт принимает привязки из чужих namespace.** У элемента появился
  список `allowedNamespaces`: чарт переводит `allowedRoutes` listener'а на эти
  namespace, и лейблы `istio.io/use-waypoint` вместе с
  `istio.io/use-waypoint-namespace` из них начинают действовать. Без списка всё
  как раньше: привязаться можно только из namespace самого вейпоинта.
- **Автомасштабирование и ресурсы вейпоинта.** `hpa` и `resources` у элемента
  уезжают в `ConfigMap` с именем `Gateway`, на который тот ссылается через
  `infrastructure.parametersRef`. Вейпоинт без этих полей рендерится как раньше,
  без `ConfigMap`. Так egress-шлюз чарта `egress-gateway` собирается из этого
  чарта, а не дублирует его шаблон.
- **Теги окружения из `global.identity`.** Когда чарт подключён сабчартом и
  ...
  самого чарта. Родитель с несколькими сабчартами задаёт их один раз, а
  дефолтные теги из `values.yaml` сабчарта не подменяют их молча.

### Changed
- **`Service` вейпоинта остаётся внутри кластера.** На `Gateway` ставится
  аннотация `networking.istio.io/service-type: ClusterIP`. Вейпоинт доступен
  только по HBONE, снаружи ему адрес не нужен.
...
### Added
- **Namespace вейпоинтов можно задать значениями.** `namespaceOverride`
  принимает готовое имя, `namespacePurpose` - только назначение, из которого имя
  собирается так же, как в чарте `namespace`. Пустые оба - как раньше, namespace
  релиза.

  Оба читаются и из `global` (`global.namespaceOverride`,
  `global.namespacePurpose`): так их передаёт родительский чарт, потому что Helm
  раздаёт `global` всем сабчартам. Своё значение чарта сильнее глобального.

  Это нужно, когда чарт подключён сабчартом к `namespace`: сабчарт не видит
  значений родителя, а namespace релиза у заказа из портала - это namespace
  назначения приложения Argo, а не имя созданного namespace. Вейпоинт уезжал
  мимо своего namespace, и заметить это можно было только в кластере.
