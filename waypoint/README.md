...
`global.namespacePurpose`) - так их передаёт родительский чарт: Helm раздаёт
`global` всем сабчартам. Своё значение чарта сильнее глобального.
...
- `kindShort` - всегда `wp`; `ConfigMap` waypoint'а носит имя своего
  `Gateway`;
...
На каждый элемент создаётся один `Gateway` класса `istio-waypoint`. Его
`Service` не выходит за пределы кластера: waypoint доступен только по HBONE.
...
| `allowedNamespaces` | нет                         | Namespace, из которых к waypoint'у можно привязаться помимо его собственного |
| `hpa`               | нет                         | Автомасштабирование waypoint'а, см. ниже                          |
| `resources`         | нет                         | Ресурсы контейнера waypoint'а                                      |

Привязка из чужого namespace (лейблы `istio.io/use-waypoint` и
`istio.io/use-waypoint-namespace` на сервисе или `ServiceEntry`) действует,
только когда listener waypoint'а допускает этот namespace. Перечислите такие
namespace в `allowedNamespaces`: чарт переведёт `allowedRoutes` на список из
собственного namespace и перечисленных. Пустой список - привязаться можно
только из namespace самого waypoint'а.

Автомасштабирование и ресурсы уезжают в `ConfigMap` с именем `Gateway`, на
который `Gateway` ссылается через `infrastructure.parametersRef`; Istio читает
его при создании `Deployment` waypoint'а. Без `hpa` и `resources` `ConfigMap`
не создаётся.
...
| `hpa.maxReplicas`        | нет, по умолчанию `5`     | Выше этого числа waypoint не поднимется     |
...
- в `allowedNamespaces` есть запись не в формате имени namespace.
