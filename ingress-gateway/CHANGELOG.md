...
- **Добавили правила размещения точек входа.** По умолчанию поды не размещаются
  на нодах, с которых внешний адрес точки входа не объявляется в сеть: на такой
  ноде точка входа работает внутри кластера, но недоступна снаружи.
  Блок `gatewayScheduling` задаёт общие значения `nodeSelector`, `tolerations`
  и `affinity` для всех точек входа заказа, а умолчание заполняет платформа.
  Своё поле в элементе `gateways[]` заменяет соответствующее умолчание целиком
  без слияния, а пустое значение, например `affinity: {}`, отключает его для
  этой точки входа. Все эти поля скрыты в форме заказа и задаются в значениях
  чарта. После обновления на 9.1.0 защита действует автоматически, менять
