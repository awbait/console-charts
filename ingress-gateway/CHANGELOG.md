...
- **У сертификата выбирается хранилище.** По умолчанию это `SecretStore`
  вашего namespace, но в записи можно назвать другое, например общий
  `ClusterSecretStore`. Платформенные сертификаты читаются из своего
  хранилища и от выбора в заказе не зависят.
