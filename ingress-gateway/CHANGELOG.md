...
- **Добавили общие правила размещения точек входа.** В блоке
  `gatewayScheduling` платформа задаёт общие для всех точек входа заказа
  значения `nodeSelector`, `tolerations` и `affinity`, скрытые в форме заказа
  и доступные через values. По умолчанию правила исключают ноды, с которых
  внешний адрес не объявляется в сеть, чтобы точка входа не оказалась
  недоступна снаружи по этой причине. После обновления на 9.2.0 защита
  действует автоматически, менять существующие заказы не нужно.
  Поле в элементе `gateways[]` заменяет одноимённое умолчание целиком, без
  слияния, а пустое значение, например `affinity: {}`, отключает это
  умолчание для отдельной точки входа.
- **Разрешения для маршрутов в другой namespace создаются автоматически.**
  Если в `xroutes[].rules[].backendRefs[]` указан `namespace`, отличный от
  namespace точки входа, чарт сам создаёт в namespace сервиса `ReferenceGrant`,
  который разрешает маршруту ссылаться на этот сервис.
  Раньше такое разрешение нужно было создавать вручную, и без него маршрут
  не пропускал трафик.
  Дополнительные настройки не нужны: разрешение появляется вместе с маршрутом
  и удаляется вместе с заказом.
  Если нет прав создавать объекты в namespace сервиса, задайте
  `referenceGrant: false` у соответствующей ссылки в `backendRefs[]` и
  договоритесь с владельцами namespace о создании `ReferenceGrant`, без
  которого маршрут не заработает.

## [9.1.0] - 2026-09-25

### Added

- **Теперь можно выбирать ноды для пода шлюза.** В values появились поля
  `gateways[].nodeSelector`, `gateways[].tolerations` и `gateways[].affinity`
  для настройки размещения пода. С их помощью можно исключить ноды, с которых
  внешний адрес шлюза не объявляется в сеть и шлюз недоступен снаружи.
  Поля задаются только в values и скрыты в форме заказа. По умолчанию они пусты,
  ноду по-прежнему выбирает планировщик, существующие заказы не затронуты.

### Changed

- **Инстанс и кластер в форме заказа не спрашиваются.** Секция «Окружение и
  проект» убрана из формы целиком: теги окружения проставляет портал, тег
  проекта по-прежнему берётся из команды заказа.

### Fixed

- Заменена малозаметная растровая иконка на SVG с контрастной подложкой и стрелкой входящего трафика.
...
- **Повтор имени в списке отклоняется формой.** У вкладок «Слушатели» и «Маршруты» объявлено, что записи различаются по имени: портал не даёт завести вторую запись с занятым именем и называет её. Раньше такой заказ принимался, а останавливался уже рендер чарта, в Argo CD, после того как изменение влито в Git. Только `view.schema.json`, чарт не менялся.
- **Заголовок «Сертификаты» в форме сертификатов не повторяется.** Действие «Редактировать сертификаты» открывало раздел, внутри которого стоял ещё один такой же заголовок, и только под ним шёл список. Теперь форма показывает сам список. Только `view.schema.json`, чарт не менялся.
- **Кнопки добавления названы по одному элементу.** На вкладках «Слушатели» и «Маршруты» кнопки читаются «Добавить слушателя» и «Добавить маршрут» вместо «Добавить Слушатели» и «Добавить Маршруты». Только `view.schema.json`, чарт не менялся.
...
- **Тег проекта берётся из команды заказа.** Поле «Проект» из формы убрано:
  ...
  **BREAKING для уже выпущенных заказов.** Тег проекта стоит в имени каждого
  ...
  другое поле, и все ресурсы поедут на новые имена: гейтвей, его ConfigMap,
  маршруты и секреты сертификатов будут созданы заново, а старые удалены. Для
  живого входа это перерыв в трафике и, если адрес не закреплён, новый адрес.

  Что сделать до обновления: пройти по выпущенным заказам и сверить
  `identity.project` с командой заказа. Где совпадает, ничего не изменится.
- Правка сертификатов переехала в меню «Действия» на вкладке «Слушатели»:
  сертификат нужен точке входа, которая расшифровывает TLS сама, и правится он
  теперь там же, где сами точки входа.

## [8.0.0] - 2026-09-09

### Added
- **Сертификат для точки входа выпускается сам.** Домен, который не описан ни
  одной записью `tls.certificates`, больше не останавливает заказ: чарт создаёт
  `Certificate`, cert-manager подписывает его у платформенного издателя и кладёт
  в namespace секрет `kubernetes.io/tls`. Обычному заказу теперь не нужно ничего,
  кроме домена у точки входа.

  Сертификат выпускается на точку входа, а не на запись: `Certificate`
  называется `{instance}-{cluster}-cert-{gateway}-{project}-{listener}`, секрет -
  `{instance}-{cluster}-secret-{gateway}-{project}-{listener}`. Две точки входа
  на одном домене получат по своему сертификату: так имя каждого секрета выведено
  из своей точки входа и не зависит от порядка списка.

  Настройки издателя лежат в `tls.issuer` и в форме заказа не показываются:
  `name`, `kind`, `group`, срок `duration` и `renewBefore`, а также `domains` -
  домены, которые издателю разрешено подписывать. Домен вне этого списка
  останавливает заказ с внятным сообщением, вместо того чтобы оставить в кластере
  сертификат, висящий в ожидании выпуска. `tls.issuer.enabled: false` выключает
  автовыпуск целиком и возвращает прежнее поведение.

- **Сертификат можно взять готовым секретом.** У записи в `tls.certificates`
  появилось поле `source`: `vault` (по умолчанию, как раньше) или `existing`.
  Запись с `existing` называет `secretName` секрета, который уже лежит в
  namespace, и чарт только ссылается на него из listener'а. Короткого имени у
  такой записи нет: секрет уже назван.

### Changed
- **BREAKING: имя точки входа теперь от 2 до 9 символов.** Раньше длина не
  ограничивалась. Имя попадает в имя автовыпущенного сертификата, а обрезка
  длинного имени по границе 63 символов молча склеила бы два разных сертификата
  в один.

  Что поправить в `values.yaml`: если у точки входа имя длиннее 9 символов,
  переименуйте её и поправьте `sectionName` в `parentRefs` маршрутов, которые
  на неё ссылаются.

- **Две точки входа одного гейтвея больше нельзя назвать одинаково.** Раньше
  такую пару отклонял только кластер, теперь рендер останавливается и называет
  обе. Имя точки входа стало частью имени сертификата, и два одинаковых имени
  дали бы один сертификат на двоих.

### Fixed
- **В README имя секрета и `ExternalSecret` записано с гейтвеем**, как их и
  собирает чарт с версии 6.0.0. В таблице имён гейтвей был потерян.
...
### Removed
- **Платформенных сертификатов больше нет.** Ключ `tls.platform` и записи
  `idptls` и `edptls` для `*.idp.ecpk.test` и `*.edp.ecpk.test` убраны: они
  читались из хранилища `vault`, а хранилище каждый заказ заводит сам чартом
  `secret-store` и называет по-своему, поэтому платформенная запись всё равно
  никуда не годилась.

  Что поправить в `values.yaml`: если listener с `tlsMode: Terminate` жил на
  домене `*.idp.ecpk.test` или `*.edp.ecpk.test` и записи в `tls.certificates`
  для него не было, добавьте её - с `path` до сертификата в StarVault и, если
  хранилище не то, что в `tls.store`, со своим `store`. Без такой записи рендер
  остановится и назовёт непокрытый hostname.

## [6.2.0] - 2026-08-27

### Changed
- **Поле «Хранилище» у сертификата принимает имена с точками**, например
  `vault.idp.ecpk.test-vault`. Так теперь может называться хранилище, которое
  заводит чарт `secret-store`, и сертификат должен уметь на него сослаться.
...
### Changed
- **Имя маршрута в кластере содержит имя его гейтвея**:
  `ed-dev-hr-edge-a-nbox-app` вместо `ed-dev-hr-nbox-app`. Раньше маршруты `app`
  на двух разных гейтвеях получали одно имя, и в кластере оставался только
  последний.

  **BREAKING**: у выпущенных заказов маршруты переедут на новые имена, то есть
  будут созданы заново, а старые удалены. Значения править не нужно.
- **Секрет сертификата тоже назван по гейтвею**: `ed-dev-secret-edge-a-nbox-cert`
  вместо `ed-dev-secret-nbox-cert`. Два заказа в одном неймспейсе называли
  сертификат одинаково (в подсказке формы стоит имя `cert`) и получали один
  секрет на двоих, из-за чего точка входа могла отдавать чужой сертификат.
  Сертификат, нужный нескольким точкам входа одного гейтвея, по-прежнему
  создаётся один раз.

  **BREAKING**: секреты и `ExternalSecret` выпущенных заказов тоже переедут на
  новые имена. Значения править не нужно.

### Added
- **Чарт останавливает заказ на именах, которые столкнутся в кластере.** Два
  гейтвея с одним именем, два маршрута одного типа с одним именем на одном
  гейтвее, две записи `tls.certificates` с одним именем - рендер падает и
  называет поле. Раньше такой заказ выпускался, а в кластере одна запись молча
  замещала другую: в случае с сертификатами точка входа могла получить чужой.
