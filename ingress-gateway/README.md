...
У маршрута между `kindShort` и проектом стоит имя его гейтвея:

```
{instance}-{cluster}-{kindShort}-{gateway}-{project}-{name}
```

Гейтвей берётся из первого `parentRefs`, поэтому одно имя маршрута можно
использовать на разных гейтвеях: `ed-dev-hr-edge-a-nbox-app` и
`ed-dev-hr-edge-b-nbox-app` - разные ресурсы. Внутри одного гейтвея имя
маршрута каждого типа остаётся уникальным, иначе рендер останавливается.
