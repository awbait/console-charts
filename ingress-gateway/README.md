...
- `name` - имя элемента секции: строчные латинские буквы, цифры и дефис.
  У Gateway от 2 до 9 символов, у маршрута и сертификата от 2 до 6.
...
Сертификат, который нужен нескольким точкам входа одного гейтвея, создаётся
один раз. Тот же сертификат на другом гейтвее - это отдельный секрет, читающий
ту же запись хранилища.
...
Список один - `tls.certificates`. Сюда добавляют свои домены: любого уровня и
не только в `ecpk.test`.
...
`tls.certificates`, первое совпадение выигрывает. Домен со звёздочкой покрывает
...
- имя Gateway короче 2 или длиннее 9 символов, имя маршрута или сертификата
  короче 2 или длиннее 6 символов;
...
- у listener'а с `tlsMode: Terminate` домен не покрыт ни одним сертификатом из
  `tls.certificates`, и не заданы `certificateRefs`;
...
- два включённых гейтвея названы одинаково, два маршрута одного типа на одном
  гейтвее названы одинаково или две записи в `tls.certificates` носят одно имя;
