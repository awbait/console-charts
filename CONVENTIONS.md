...
`gateways[].name` и имя точки входа. Такое послабление описывается в схеме
отдельным определением рядом с `shortToken` и повторяется в хелпере имени,
чтобы схема и рендер говорили одно и то же.
