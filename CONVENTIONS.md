...
- `icon` (опц.) - data URI с PNG или SVG, показывается в каталоге портала
  (см. `ingress-gateway` - PNG, `secret-store` - SVG). Векторный знак берём SVG:
  он весит около килобайта против четырёх у растрового и не мылится на крупной
  карточке.
...
если 6 не хватает на осмысленное имя: в `ingress-gateway` до 9 занимают
`gateways[].name` и имя точки входа, а `egress-gateway` поднял до 9 сам
`shortToken`, то есть имена всех своих элементов. Такое послабление описывается
в схеме отдельным определением рядом с `shortToken` (или правкой самого
`shortToken`, когда оно касается всех элементов чарта) и повторяется в хелпере
имени, чтобы схема и рендер говорили одно и то же.

Поднимая границу, посчитайте самое длинное имя, которое теперь складывается из
частей, и убедитесь, что оно не упирается в `trunc 63`: обрезка не ошибка, а
молчаливое склеивание двух разных имён в одно.

Когда ресурс назван не по своему элементу, а по чужому (в `ingress-gateway`
автовыпущенный сертификат назван по точке входа, которая его использует),
граница передаётся в хелпер имени параметром: она принадлежит источнику имени,
а не типу ресурса.
...
- **Код уникален на Kind, но привязка к Kind - в пределах чарта.** Один чарт сам
  ...
  `ConfigMap` свой код `cm`, а в `waypoint` `ConfigMap` наследует код `wp`
  своего Gateway (один логический объект - вейпоинт).
...
identity-labels (см. «Стандартные labels»). Чарт, который подключают
сабчартом, читает ещё и `global.identity` и предпочитает его своему блоку,
чтобы родитель задавал теги один раз на все сабчарты (см. правило про
`global` в разделе про схему).
...
- **`global` принадлежит Helm, своих флагов туда не кладём.** Блок описывается
  открытым `type: object`: что положит родитель, чарт не знает. `namespace` и
  `project` держали в нём свой выключатель и из-за этого отвергали настоящий
  global. Исключение - то, что родитель раздаёт сабчартам и что они читают
  по договорённости: `global.identity` (теги окружения, сильнее блока
  `identity` сабчарта, когда родитель его задал; так читают `namespace` и
  `waypoint`) и `global.namespacePurpose` (назначение неймспейса). Составной
  чарт, у которого несколько сабчартов собирают имена из одних тегов
  (`egress-gateway`), держит свой `identity` прямо в `global.identity` и
  описывает его в схеме там.
