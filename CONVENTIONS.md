...
- **`global` принадлежит Helm, своих флагов туда не кладём.** Блок описывается
  ...
  и не читает. Собственный выключатель чарта - корневой `enabled` (default
  `true`), его и читают шаблоны. `namespace` и `project` держали выключатель в
  `global.enable` и из-за этого отвергали настоящий global.
