...
Язык: Markdown-документы (`README.md`, `CHANGELOG.md`, этот файл) и комментарии в
`values.yaml` - на русском. Комментарии внутри `templates/` (Go-шаблоны
`{{/* ... */}}`) - на английском, без кириллицы.
...
- при необходимости - краткая текстовая подсказка по следующему шагу (например,
  как привязать namespace к waypoint), без `kubectl`-команд.
